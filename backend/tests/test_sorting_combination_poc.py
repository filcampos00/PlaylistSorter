...
SAMPLE_PLAYLIST_DATA = [
    ...
    TrackData(
        "v07", "Starboy", "The Weeknd", "Starboy", "2016-11-18", 1, 230000
    ),  # Same day as Bruno!
    TrackData(
        "v08", "I Feel It Coming", "The Weeknd", "Starboy", "2016-11-18", 18, 269000
    ),
    # === SAME ARTIST, MULTIPLE ALBUMS, SAME TRACK NUMBERS ===
    TrackData(
        "v09", "We Will Rock You", "Queen", "News of the World", "1977-10-28", 1, 122000
    ),  # Track 1
    TrackData(
        "v10",
        "Somebody to Love",
    ...
    TrackData(
        "v12", "Blinding Lights", "The Weeknd", "After Hours", "2020-03-20", 9, 200000
    ),
    TrackData(
        "v13",
        "Blinding Lights",
    ...
    # === IDENTICAL DURATIONS ===
    TrackData(
        "v14", "Save Your Tears", "The Weeknd", "After Hours", "2020-03-20", 6, 215000
    ),
    TrackData(
        "v15", "In Your Eyes", "The Weeknd", "After Hours", "2020-03-20", 7, 215000
    ),  # Same duration!
    # === Edge cases ===
    TrackData(
        "v16", "Uptown Funk", "Bruno Mars", "Uptown Special", "2015-01-13", 4, 270000
    ),
    TrackData(
        "v17", "That's What I Like", "Bruno Mars", "24K Magic", "2016-11-18", 3, 206000
    ),
    TrackData(
        "v18", "Mystery Song", "Unknown Artist", None, None, 1, 180000
    ),  # Missing album/date
    TrackData(
        "v19",
        "Love of My Life",
...
class TestUnusualButValidCombinations:
    ...
    def test_title_then_album_date_uses_tiebreaker(self, sample_playlist):
        ...
        assert dates == ["1975-11-21", "2016-11-18", "2020-03-20"], (
            "Intros should be ordered by date"
        )
...
class TestSortingDepthRecommendations:
    ...
    def test_diminishing_returns_past_4_levels(self, sample_playlist):
        ...
        same_order = all(
            r4.video_id == r5.video_id for r4, r5 in zip(result_4, result_5)
        )

        # With our sample data, track numbers within same album are unique
        # so title as L5 never breaks any ties
        assert same_order, (
            "5th level should have no effect when 4 levels are deterministic"
        )
